...
;(function () {
    ...
    try {
      ...
      document.addEventListener(
        ...
        function (ev) {
          try {
            const btn =
              ev.target && (ev.target.closest ? ev.target.closest('.ec-preset-btn') : null)
            if (!btn) return
            ev.preventDefault()
            ev.stopPropagation()
      ...
      return fetch('https://api.eclesiar.com/account/equips/build', {
        ...
        if (res && res.ok) {
          try {
            ...
            const sel =
              `.ec-preset-btn[data-buildid="${bid}"]` + `, .ec-preset-btn[data-build="${bid}"]`
        ...
        if (origFetch) {
          window.fetch = function () {
            try {
              ...
              if (init.headers) {
                const ent = init.headers.entries
  ...
  function detectLanguageHeuristic() {
    ...
    try {
      ...
      const goldText = norm(
        document.querySelector('.premium-finances .text')?.textContent ||
      ...
      const accTitle = norm(
        document.querySelector('#account_level_sidebar .title')?.textContent || ''
      )
  ...
  function updateStorageMenuLabel(retry = 0) {
    try {
      ...
      const desiredLabel =
        TRANSLATIONS[LANGUAGE]?.storageMenu ||
        (LANGUAGE === 'pl' ? 'Pawlacz' : storageMenuItem.textContent)
    ...
    try {
      ...
      setTimeout(() => {
        try {
          ...
          else if (document.referrer && !/\/training(\b|$)/.test(document.referrer))
            window.history.back()
        } catch {}
      }, 200)
    } catch {}
  ...
  function ensurePresetToolbarInBuildingModal() {
    try {
      ...
      if (!modal) return
      if (
        modal.querySelector('#ec-toolbar-building') ||
  ...
  function ensurePresetToolbarInMining() {
    try {
      ...
      if (!parent) return
      if (
        parent.querySelector('#ec-toolbar-mining') ||
  ...
  function formatCompactDate(dt) {
    try {
      ...
      // Compact display: day.month hour:min (no year, 24h format)
  ...
  function displayEnergyFullTime() {
    ...
    console.log('Game stats element:', gameStatsElement)
    ...
    const energyTimeFull = new Date(currentTime.getTime() + energyTimeLeft * 1000)
    const foodTimeFull = new Date(currentTime.getTime() + foodTimeLeft * 1000)
    ...
    if (energyBar) {
      ...
      if (energyValue) {
        ...
        fullTimeSpan.textContent = `${
          TRANSLATIONS[LANGUAGE]?.energyShort || ''
        } ${formatCompactDate(energyTimeFull)} (${formatRelative(energyTimeFull)})`
    ...
    log('Energy and food time info integrated into bars')
  }
